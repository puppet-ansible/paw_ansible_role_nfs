...
# Puppet task for executing Ansible role: ansible_role_nfs
# This script runs the entire role via ansible-playbook

# Determine the ansible modules directory
if [ -n "$PT__installdir" ]; then
  ANSIBLE_DIR="$PT__installdir/lib/puppet_x/ansible_modules/ansible_role_nfs"
else
  # Fallback to /opt/puppetlabs/puppet/cache/lib/puppet_x/ansible_modules
  ANSIBLE_DIR="/opt/puppetlabs/puppet/cache/lib/puppet_x/ansible_modules/ansible_role_nfs"
fi
...
if [ -d "$ANSIBLE_DIR/roles" ] && [ -f "$ANSIBLE_DIR/roles/paw_ansible_role_nfs/playbook.yml" ]; then
  # Collection structure
  PLAYBOOK_PATH="$ANSIBLE_DIR/roles/paw_ansible_role_nfs/playbook.yml"
  PLAYBOOK_DIR="$ANSIBLE_DIR/roles/paw_ansible_role_nfs"
  ...
  echo "{\"_error\": {\"msg\": \"playbook.yml not found in $ANSIBLE_DIR or $ANSIBLE_DIR/roles/paw_ansible_role_nfs\", \"kind\": \"puppet-ansible-converter/playbook-not-found\"}}"
  exit 1
fi
...
if [ -n "$PT_export" ]; then
  ...
  EXTRA_VARS="$EXTRA_VARS\"export\": \"$PT_export\""
fi
if [ -n "$PT_nfs_exports" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
  fi
  EXTRA_VARS="$EXTRA_VARS\"nfs_exports\": \"$PT_nfs_exports\""
fi
if [ -n "$PT_nfs_rpcbind_state" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
  fi
  EXTRA_VARS="$EXTRA_VARS\"nfs_rpcbind_state\": \"$PT_nfs_rpcbind_state\""
fi
if [ -n "$PT_nfs_rpcbind_enabled" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
  fi
  EXTRA_VARS="$EXTRA_VARS\"nfs_rpcbind_enabled\": \"$PT_nfs_rpcbind_enabled\""
fi
EXTRA_VARS="$EXTRA_VARS}"
...
if [ $EXIT_CODE -eq 0 ]; then
  echo '{"status": "success", "role": "ansible_role_nfs"}'
else
  echo "{\"status\": \"failed\", \"role\": \"ansible_role_nfs\", \"exit_code\": $EXIT_CODE}"
fi
