...
if ($env:PT__installdir) {
  $AnsibleDir = Join-Path $env:PT__installdir "lib\puppet_x\ansible_modules\ansible_role_nfs"
} else {
  # Fallback to Puppet cache directory
  $AnsibleDir = "C:\ProgramData\PuppetLabs\puppet\cache\lib\puppet_x\ansible_modules\ansible_role_nfs"
}
...
$CollectionPlaybook = Join-Path $AnsibleDir "roles\paw_ansible_role_nfs\playbook.yml"
...
if ((Test-Path (Join-Path $AnsibleDir "roles")) -and (Test-Path $CollectionPlaybook)) {
  ...
  $PlaybookDir = Join-Path $AnsibleDir "roles\paw_ansible_role_nfs"
  ...
  $result = @{
    _error = @{
      msg = "playbook.yml not found in $AnsibleDir or $AnsibleDir\roles\paw_ansible_role_nfs"
...
$ExtraVars = @{}
if ($env:PT_export) {
  $ExtraVars['export'] = $env:PT_export
}
if ($env:PT_nfs_exports) {
  $ExtraVars['nfs_exports'] = $env:PT_nfs_exports
}
if ($env:PT_nfs_rpcbind_state) {
  $ExtraVars['nfs_rpcbind_state'] = $env:PT_nfs_rpcbind_state
}
if ($env:PT_nfs_rpcbind_enabled) {
  $ExtraVars['nfs_rpcbind_enabled'] = $env:PT_nfs_rpcbind_enabled
}
